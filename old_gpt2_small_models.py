!pip install torch datasets loguru numpy transformers
...
class ModelConfig:
    ...
    batch_size: Optional[int] = (
        None  # Will be calculated based on memory share
    )
...
class PerformanceTracker:
    ...
    def __init__(self, window_size: int = 5):
        ...
        logger.info(
            f"Initialized PerformanceTracker with window_size={window_size}"
        )
    ...
    def calculate_learning_rate(self, model_name: str) -> float:
        ...
        if model_name not in self.eval_scores:
            logger.warning(
                f"No evaluation scores available for {model_name}"
            )
            return 0.0
        ...
        if len(scores) < 2:
            logger.info(
                f"Not enough data to calculate learning rate for {model_name}"
            )
            return 0.0
        ...
        normalized_improvement = avg_improvement / (
            1.0 + abs(recent_scores[-1])
        )
        ...
        logger.info(
            f"Model {model_name} learning rate: {normalized_improvement:.6f}"
        )
    ...
    def get_memory_allocation_weights(self) -> Dict[str, float]:
        ...
        learning_rates = {
            name: rates[-1] if rates else 0.0
        ...
        if all(rate <= 0 for rate in learning_rates.values()):
            logger.warning(
                "All models have negative or zero learning rates, using default allocation"
            )
            # Default to equal allocation
            equal_share = (
                1.0 / len(learning_rates) if learning_rates else 1.0
            )
            return {name: equal_share for name in learning_rates}

        # Convert negative learning rates to small positive values
        min_positive_rate = min(
            (rate for rate in learning_rates.values() if rate > 0),
            default=1e-6,
        )
        adjusted_rates = {
            name: max(rate, min_positive_rate * 0.1)
        ...
        weights = {
            name: rate / total
            for name, rate in adjusted_rates.items()
        }

        # Apply smoothing to avoid extreme allocation changes
        current_shares = {
            name: shares[-1] if shares else 1.0 / len(learning_rates)
            for name, shares in self.memory_shares.items()
        }

        smoothed_weights = {}
        smoothing_factor = (
            0.7  # How much to consider the current allocation
        )
        ...
        logger.info(
            f"Memory allocation weights: {json.dumps(log_data)}"
        )
...
class MemoryManager:
        ...
        if total_memory_gb is None:
            if torch.cuda.is_available():
                ...
                self.total_memory = int(
                    total_memory * 0.9
                )  # 90% of total memory in bytes
                total_memory_gb = self.total_memory / (1024**3)
                logger.info(
                    f"Detected {total_memory_gb:.2f} GB GPU memory, using 90% for training"
                )
            else:
                ...
                total_memory_gb = 8
                logger.info(
                    f"No GPU detected, using default {total_memory_gb} GB memory limit"
                )
        else:
            self.total_memory = int(
                total_memory_gb * (1024**3)
            )  # Convert GB to bytes

        self.model_memory_usage: Dict[str, int] = {}
        self.model_batch_sizes: Dict[str, int] = {}
        logger.info(
            f"Initialized MemoryManager with {total_memory_gb:.2f} GB total memory"
        )

    def estimate_model_memory(
        self, model: nn.Module, sample_input_size: Tuple[int, int]
    ) -> int:
        """
        ...
        if torch.cuda.is_available():
            ...
            sample_input = torch.randint(
                0, 1000, (batch_size, seq_len), device="cuda"
            )
            ...
            logger.info(
                f"Estimated memory for batch size {batch_size}: {total_estimate / (1024**2):.2f} MB"
            )
            ...
            logger.info(
                f"Estimated CPU memory: {memory_estimate / (1024**2):.2f} MB"
            )
    ...
    def calculate_batch_size(
        ...
        logger.info(
            f"Memory available for {model_name}: {available_memory / (1024**2):.2f} MB"
        )
        ...
        while low <= high:
            mid = (low + high) // 2
            memory_required = self.estimate_model_memory(
                model, (mid, sequence_length)
            )
...
class ModelArena:
    ...
    def __init__(
        ...
        self.memory_reallocation_interval = (
            memory_reallocation_interval
        )
        ...
        self.global_step = 0
        self.model_steps = {
            config.name: 0 for config in model_configs
        }
        ...
        logger.info(
            f"Initialized ModelArena with {len(model_configs)} models"
        )
        logger.info(f"Using device: {self.device}")
        for config in model_configs:
            logger.info(
                f"Model: {config.name}, Type: {config.model_type}, ID: {config.model_id}"
            )
    ...
    def _load_dataset(self):
        ...
        logger.info(
            f"Dataset loaded: {len(dataset['train'])} training examples"
        )
        logger.info(
            f"Validation set: {len(dataset['validation'])} examples"
        )

    def _initialize_model(self, config: ModelConfig):
        """Initialize a model and its components based on its configuration."""
        logger.info(
            f"Initializing model {config.name} ({config.model_type})"
        )
        ...
        self.train_dataloaders[config.name] = DataLoader(
            tokenized_train, batch_size=1, shuffle=True
        )

        self.eval_dataloaders[config.name] = DataLoader(
            tokenized_eval, batch_size=1, shuffle=False
        )
    ...
    def _allocate_memory(self):
        ...
        if any(self.performance_tracker.learning_rates.values()):
            ...
            weights = (
                self.performance_tracker.get_memory_allocation_weights()
            )
        else:
            # Initial allocation based on configs
            weights = {
                config.name: config.initial_memory_share
                for config in self.model_configs
            }
        ...
        for config in self.model_configs:
            ...
            self.performance_tracker.add_memory_share(
                config.name, config.current_memory_share
            )
        ...
        for config in self.model_configs:
            ...
            tokenized_train = self.train_dataloaders[
                config.name
            ].dataset
            tokenized_eval = self.eval_dataloaders[
                config.name
            ].dataset
            ...
            self.eval_dataloaders[config.name] = DataLoader(
                tokenized_eval, batch_size=batch_size, shuffle=False
            )

            logger.info(
                f"Model {config.name}: Memory share = {config.current_memory_share:.4f}, "
    ...
    def _evaluate_model(self, model_name: str) -> float:
        ...
        logger.info(
            f"Model {model_name} evaluation: Loss = {avg_loss:.4f}, Score = {score:.4f}"
        )
        return score
    ...
    def train(
        ...
        self.global_step = 0
        self.model_steps = {
            config.name: 0 for config in self.model_configs
        }
        model_iterators = {
            config.name: iter(self.train_dataloaders[config.name])
            for config in self.model_configs
        }
        ...
        for config in self.model_configs:
            self.performance_tracker.calculate_learning_rate(
                config.name
            )
        ...
        while self.global_step < total_steps:
            ...
            for config in self.model_configs:
                ...
                steps_to_train = max(
                    1, int(config.current_memory_share * 10)
                )
                ...
                for _ in range(steps_to_train):
                    ...
                    try:
                        ...
                        model_iterators[model_name] = iter(
                            self.train_dataloaders[model_name]
                        )
                        batch = next(model_iterators[model_name])
                ...
                self.performance_tracker.add_loss(
                    model_name, avg_loss
                )
                logger.info(
                    f"Model {model_name} step {self.model_steps[model_name]}: "
                    f"Loss = {avg_loss:.4f}, "
            ...
            if self.global_step % self.evaluation_interval == 0:
                logger.info(
                    f"Global step {self.global_step}: Evaluating all models"
                )
                for config in self.model_configs:
                    self._evaluate_model(config.name)
                    self.performance_tracker.calculate_learning_rate(
            ...
            if (
                self.global_step % self.memory_reallocation_interval
                == 0
            ):
                logger.info(
                    f"Global step {self.global_step}: Reallocating memory"
                )
                self._allocate_memory()
    ...
    def _save_checkpoints(self, save_dir: str, step: int):
        ...
        training_state = {
            "global_step": step,
            "model_steps": {
                config.name: self.model_steps[config.name]
                for config in self.model_configs
            },
            "model_configs": [
                vars(config) for config in self.model_configs
            ],
            "performance_tracker": {
        ...
        with open(
            os.path.join(checkpoint_dir, "training_state.json"), "w"
        ) as f:
            json.dump(training_state, f, indent=2)

        logger.info(
            f"Saved checkpoints at step {step} to {checkpoint_dir}"
        )
